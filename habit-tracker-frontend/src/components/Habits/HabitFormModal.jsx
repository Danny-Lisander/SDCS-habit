...
function HabitFormModal({ isOpen, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await createHabit(form); // отправка запроса на backend
      onSave(response.data); // добавляем привычку в список на фронте
      setForm({
        title: "",
        description: "",
        category: "",
  ...
  return (
    <div style={overlayStyle}>
      <form onSubmit={handleSubmit} style={modalStyle}>
        ...
        <button
          type="submit"
          style={{ ...buttonStyle, backgroundColor: "#ffa94d" }}
        >
          Сохранить
        </button>
        <button
          type="button"
          onClick={onClose}
          style={{ ...buttonStyle, backgroundColor: "#eee", color: "#333" }}
        >
          Отмена
        </button>
      </form>
